  ...
  const submit = () => {
    if(publishEnable) {
        const newSubscription = {
            name: name,
            price: Number(price),
            curreny: CONSTANT.CURRENCIES[option],
            benefits: benefits,
            description: description
        }

        dispatch(subScriptionAction.saveSubscription(newSubscription))
    }
  }
  ...
  return (
    <div className="set-subscription-wrapper">
      <div className="page-header">
        <div onClick={() => navigate(`/${user?.personalisedUrl}?tab=subscription`)}>
      ...
      <div className="set-subscription">
        <div className="subscription-detail">
          ...
          <div style={{ marginTop: '20px' }}>
            ...
            ({editorState === null ? 0 : editorState.getCurrentContent().getPlainText().length}/300 characters)
          </div>
          </div>
        </div>
        <div className="subscription-detail">
            ...
            <div style={{ marginBottom: '30px' }}>
                {benefits.map((benefit: any, index: any) => (
                    <div key={index} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px' }}>
                        ...
                        <div className={`remove-btn ${index === 0 ? 'inactive' : 'active'}`} onClick={() => removeBenefit(index)}>
                            <RemoveIcon color={index === 0 ? "#E1E0DF" : "#DE5A67" } width={20} />
                        </div>
                    </div>
                ))}
            </div>
