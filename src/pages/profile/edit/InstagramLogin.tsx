...
export const InstagramLogin = () => {
    ...
    const onSuccess = useCallback(
        async (code) => {
            const isLocal = true
            const redirect_uri = () => {
                const { protocol, host, pathname, origin } = window.location
                if (!isLocal) return origin + pathname
                const newProtocol = protocol === 'http' ? 'http' : 'https'
                return `${newProtocol}://${host}${pathname}`
            };
            const meta = {
                code,
    ...
    }, [onSuccess, onFailure, searchParams])

    return (
        <>
            {hasInstagram && <div onClick={openInstagramProfile}>View</div>}
            <div className={hasInstagram ? 'remove-btn' : 'connect-btn'}
