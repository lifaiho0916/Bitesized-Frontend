...
const AdminComment = (props: any) => {
  ...
  const displayTime = (left: any) => {
    ...
    let res: any = "Posted";
    if (Math.floor(passTime / (3600 * 24 * 30)) >= 1)
      res =
        res +
        " " +
        Math.floor(passTime / (3600 * 24 * 30)) +
        "" +
        (Math.floor(passTime / (3600 * 24 * 30)) === 1
          ? contexts.ITEM_CARD.MONTH
          : contexts.ITEM_CARD.MONTHS);
    else if (Math.floor(passTime / (3600 * 24 * 7)) >= 1)
      res =
        res +
        " " +
        Math.floor(passTime / (3600 * 24 * 7)) +
        "" +
        (Math.floor(passTime / (3600 * 24 * 7)) === 1
          ? contexts.ITEM_CARD.WEEK
          : contexts.ITEM_CARD.WEEKS);
    else if (Math.floor(passTime / (3600 * 24)) >= 1)
      res =
        res +
        " " +
        Math.floor(passTime / (3600 * 24)) +
        "" +
        (Math.floor(passTime / (3600 * 24)) === 1
          ? contexts.ITEM_CARD.DAY
          : contexts.ITEM_CARD.DAYS);
    else if (Math.floor(passTime / 3600) >= 1)
      res =
        res +
        " " +
        Math.floor(passTime / 3600) +
        "" +
        (Math.floor(passTime / 3600) === 1
          ? contexts.ITEM_CARD.HOUR
          : contexts.ITEM_CARD.HOURS);
    else if (Math.floor(passTime / 60) > 0)
      res =
        res +
        " " +
        Math.floor(passTime / 60) +
        "" +
        (Math.floor(passTime / 60) === 1
          ? contexts.ITEM_CARD.MIN
          : contexts.ITEM_CARD.MINS);
  ...
  const localPrice = useMemo(() => {
    if (currencyRate && user && bite.owner) {
      const rate =
        bite.currency === "usd" ? 1.0 : currencyRate[`${bite.currency}`];
      const usdAmount = bite.price / rate;
      const rate1 =
        user.currency === "usd" ? 1.0 : currencyRate[`${user.currency}`];
  ...
  const isOwner = useMemo(() => {
    if (user && bite.owner && String(user.id) === String(bite.owner._id))
      return true;
    else return false;
  }, [user, bite]);
  ...
  return (
    <div className="admin-comment-wrapper">
      <div className="page-header">
        <div onClick={() => navigate("/admin/comment")}>
      ...
      <DelCommentModal
        ...
        handleSubmit={() => {
          ...
          dispatch(biteAction.deleteComment(bite?._id, deleteIndex));
        }}
      />
          ...
          <div className="avatar-part">
            <div className="avatar-title">
              <div className="left-time">
                <ClockIcon color="#DE5A67" width={18} height={18} />
                &nbsp;<span>{displayTime(bite?.time)}</span>
              </div>
              <div className="avatar">
                {bite.owner && (
                  <Avatar
                    avatar={
                      bite.owner.avatar.indexOf("uploads") !== -1
                        ? `${process.env.REACT_APP_SERVER_URL}/${bite.owner.avatar}`
                        : bite.owner.avatar
                    }
              ...
              <div className="status-chip">
                <div className={`chip ${bite.currency ? "paid" : "free"}`}>
                  {bite.currency
                    ? user
                      ? getLocalCurrency(user.currency) + localPrice
                      : ""
                    : "Free"}
                </div>
          ...
          <div className="thumbnail-part">
            {bite.videos.map((video: any, index: any) => (
              <div
                ...
                className="thumbnail"
                style={{
                  backgroundColor: bite.currency ? "#D8F7D8" : "#FBBEB1",
                }}
              >
                <img
        ...
        {bite.title && (
          <div className="comment-part">
            <div className="section-header">
              <CommentIcon color="#EFA058" width={30} height={30} />
              <span>Comments</span>
            </div>
                ...
                <div className="bubble-part scroll-bar-lg" id="scroll">
                  {bite.comments.map((comment: any, index: any) => (
                    <div
                      ...
                      style={
                        user &&
                        String(comment.commentedBy._id) === String(user.id)
                          ? { marginLeft: "auto" }
                          : {}
                      }
                    >
