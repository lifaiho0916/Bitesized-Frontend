...
export const accountAction = {
	...
	addInstagramAccount: (socialInfo: any) => async (dispatch: Dispatch<any>) => {
		try {
			dispatch({ type: SET_LOADING_TRUE });
			const response = await api.addInstagramAccount(socialInfo);
			const { data } = response;
			...
			if (data.success) {
				...
				alert("Instagram account is connected successfully");
				dispatch({ type: SET_SOCIAL_ACCOUNT, payload: payload.socialAccount });
			}
		} catch (error) {
			dispatch({ type: SET_LOADING_FALSE });
